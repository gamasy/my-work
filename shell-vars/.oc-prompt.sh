...
{
    # Get current context
    CONTEXT=$(oc config view -o "jsonpath={.contexts[?(@.name==\"$(oc config view -o "jsonpath={\$.current-context}")\")].context.namespace}")
...
alias o='oc'
alias node='oc get node'
alias nodew='oc get node -o wide'
alias pod='oc get pods'
alias svc='oc get svc'
zlias ocw='oc whoami'
alias deploy='oc get deployment'
alias repset='oc get replicaset'
alias daemon='oc get daemonsets'
alias createf='oc create -f'
alias project='oc get projects'
alias podw='oc get pods -o wide'
alias login="oc login -u rhadmin -p 'R3dH4t!1'"
alias k9s=~/k9s.sh
...
forloop(){     for node in $(oc get nodes -o jsonpath='{.items[*].status.addresses[?(@.type=="InternalIP")].address}'); do         echo $node; ssh  -o ConnectTimeout=5 -i  key  core@${node} ""$@""; echo "@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@";     done; }
